...
$(function() {
    function f(a, b) {
        localStorage && localStorage.setItem("mahua_" + a, b)
    }
    function d(a, b) {
        return localStorage ? null == localStorage.getItem("mahua_" + a) ? void 0 != b ? b: null: localStorage.getItem("mahua_" + a) : void 0 != b ? b: null
    }
    function l() {
    ...
    function t(a) {
        ...
        if (null != a) {
            ...
            $.extend(h, b);
            n(a);
            $("#mahua-changeSession").append('<option value="' + a + '" selected>' + a + "</option>");
            c.focus()
        }
    }
    function H(a) {
        h[a].buildIn ? alert("内置的不能删除, 其实不应该让你看到删除滴。。。") : (G(a), localStorage && localStorage.removeItem("markContent_" + a), $("#mahua-changeSession option").filter(function() {
            return $(this).val() == e
        }).remove(), m--, n("MaHua简介"))
    }
    function n(a) {
    ...
    function u(a) {
        $("#mahua-docname").text(a);
        a = $(window).width() - $("#mahua-docname").outerWidth(!0);
        $("#mahua-docname").css("left", a / 2)
    }
    function o(a) {
        window.iframeDoc = g;
    ...
    function C(a) {
        $("#mahua-dragMask").size() || $('<div id="mahua-dragMask"></div>').appendTo($("body"));
        $("#mahua-dragMask").show().css({
            left: 0,
            top: 0,
            width: $(window).width(),
        ...
        150 < a.pageX && 150 < $(window).innerWidth() - a.pageX && ($("#mahua-spliter").css("left", a.clientX), window.getSelection ? window.getSelection().removeAllRanges() : document.selection.empty())
    }
    function D() {
        ...
        $("#mahua-editor").css("width", a);
        $("#mahua-result").css("width", $(window).innerWidth() - parseInt(a));
        $("#mahua-result").css("marginLeft", a);
        f("left", a)
    }
    var c = ace.edit("mahua-editor");
    ...
    y($("#mahua-linenum").prop("checked") ? 1 : 0);
    $("#mahua-syncRoll").prop("checked", parseInt(d("curSyncRoll", 0)));
    $("#mahua-editor").css("width", d("left", "50%"));
    $("#mahua-result").css("width", d("left") ? $(window).innerWidth() - parseInt(d("left")) : "50%");
    $("#mahua-result").css("marginLeft", d("left", "50%"));
    $("#mahua-spliter").css("left", d("left", "50%"));
    w(p);
    $(k).on("load",
    function() {
        g = k.contents();
    ...
    function(a, b) {
        $("#mahua-editorThemes").append('<option value="' + b + '" ' + (b == p ? "selected": "") + ">" + b + "</option>")
    });
    ...
    $.each(h,
    function(a) {
        $("#mahua-changeSession").append('<option value="' + a + '" ' + (a == e ? "selected": "") + ">" + a + "</option>")
    });
    $("#mahua-keyboard").click(function() {
        x($(this).prop("checked"))
    });
    $("#mahua-linenum").click(function() {
        y($(this).prop("checked") ? 1 : 0)
    });
    $("#mahua-syncRoll").click(function() {
        q = $(this).prop("checked") ? 1 : 0;
        f("syncRoll", q)
    });
    $("#mahua-editorThemes").change(function() {
        w(this.value)
    });
    ...
    $("#mahua-removeSession").click(function() {
        H(e)
    });
    $("#mahua-toggleMode").click(function() {
    ...
    $("#mahua-attachInfo").click(function() {
        E = $(this).prop("checked")
    });
    $("#mahua-config").click(function() {
        $("#mahua-toolbar").slideDown()
    });
    $("#mahua-closeConfig").click(function() {
        $("#mahua-toolbar").slideUp()
    });
    $("#mahua-session").click(function() {
        t()
    });
    ...
    $("#mahua-closeDonate").click(function() {
        $("#mahua-donate-detail").slideUp()
    });
